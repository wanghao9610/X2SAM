...
class Sam2ImageProcessorFast(BaseImageProcessorFast):
    ...
    def preprocess(
        ...
        **kwargs: Unpack[Sam2FastImageProcessorKwargs],
        ...
        """
        return super().preprocess(images, mask_labels, **kwargs)

    def _preprocess_image_like_inputs(
        self,
        images: ImageInput,
        mask_labels: Optional[ImageInput],
        do_convert_rgb: bool,
        ...
        **kwargs: Unpack[Sam2FastImageProcessorKwargs],
        ...
        if mask_labels is not None:
            processed_mask_labels = self._prepare_image_like_inputs(
                images=mask_labels,
                expected_ndims=2,
            ...
            mask_labels_kwargs = kwargs.copy()
            mask_labels_kwargs.update(
                {
                    "do_normalize": False,
                    "do_rescale": False,
                    "interpolation": pil_torch_interpolation_mapping[PILImageResampling.NEAREST],
                    "size": mask_labels_kwargs.pop("mask_size"),
                }
            )
            processed_mask_labels = self._preprocess(images=processed_mask_labels, **mask_labels_kwargs)
            data["labels"] = processed_mask_labels.squeeze(1).to(torch.int64)
