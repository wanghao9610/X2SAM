...
def get_length_grouped_indices(lengths, group_batch_size, generator=None):
    def process(lengths, group_batch_size, generator=None):
        ...
        megabatches = [indices[i : i + group_batch_size].tolist() for i in range(0, len(lengths), group_batch_size)]
...
class LengthGroupedSampler(Sampler):
    def __init__(
        ...
        print_log(
            f"LengthGroupedSampler construction is complete, " f"and the selected attribute is {length_property}",
            logger="current",
        )
