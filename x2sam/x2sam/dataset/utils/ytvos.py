# reference: https://github.com/youtubevos/cocoapi/blob/master/PythonAPI/pycocotools/ytvos.py
...
class YTVOS:
    def __init__(self, annotation_file=None, dataset=None):
        ...
        if not annotation_file == None:
            ...
            self.createIndex()
        elif not dataset == None:
            self.dataset = dataset
            self.createIndex()
        else:
            raise ValueError("Either annotation_file or dataset must be provided")
    ...
    def getAnnIds(self, vidIds=[], catIds=[], areaRng=[], iscrowd=None):
        ...
        if len(vidIds) == len(catIds) == len(areaRng) == 0:
            ...
            anns = (
                anns
                if len(areaRng) == 0
                else [ann for ann in anns if ann["avg_area"] > areaRng[0] and ann["avg_area"] < areaRng[1]]
            )
        if not iscrowd == None:
