...
from x2sam.structures import BitMasks, Boxes, BoxMode, Keypoints, PolygonMasks
...
class Visualizer:
    ...
    def draw_predictions(
        ...
        elif "objseg" in data_name:
            output_img, aux_output_img = self.draw_objseg(**kwargs)
        else:
    ...
    def draw_objseg(
        ...
        **kwargs,
        ...
        Returns:
            output (VisImage): image object with visualizations.
        """
        if len(phrases) == 0 or phrases is None or phrases[0] == "":
            phrases = [f"vprompt_{i}" for i in range(len(vprompt_masks))]
        if isinstance(segmentation, torch.Tensor):
            segmentation = segmentation.numpy()
        if vprompt_masks is not None and isinstance(vprompt_masks, torch.Tensor):
            vprompt_masks = vprompt_masks.numpy()
        labels, areas = np.unique(segmentation, return_counts=True)
        sorted_idxs = np.argsort(-areas).tolist()
        labels = labels[sorted_idxs]
        score = segments_info["score"]
        for i, label in enumerate(filter(lambda l: l != self.metadata.get("ignore_value", 255), labels)):
            text = f"{phrases[i]} {score:.2f}"
            text = "\n".join(textwrap3.wrap(text, width=32))
            binary_mask = (segmentation == label).astype(np.uint8)
            self.draw_binary_mask(
                binary_mask,
