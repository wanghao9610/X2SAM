...
class Sam2VideoInferenceSession:
    ...
    def __init__(
        ...
        self.processed_frames = (
            dict(enumerate(videos.to(video_storage_device, dtype=dtype))) if videos is not None else None
        )
...
class Sam2ImageInferenceSession:
    def __init__(
        self,
...
class Sam2ImageSegmentationOutput(ModelOutput):
    ...
    image_embeds (`tuple(torch.FloatTensor)`):
...
@dataclass
@auto_docstring(custom_intro="Base class for the Sam2 mask decoder's output.")
class Sam2MaskDecoderOutput(ModelOutput):
    r"""
    Args:
        ...
        masks_queries_logits (`tuple(torch.FloatTensor)`):
            Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_masks, num_query_tokens, height, width)`.
            The logits of the predicted masks.
        intermediate_hidden_states (`tuple(torch.FloatTensor)`):
            Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_masks, num_query_tokens, hidden_size)`.
            The intermediate hidden states of the model.
    """
    ...
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None
    multi_masks: torch.FloatTensor = None
    iou_pred: torch.FloatTensor = None
    sam_tokens_out: torch.FloatTensor = None
        ...
        for i, block_module in enumerate(self.blocks):
            if self.gradient_checkpointing and self.training:
                ...
                hidden_states = block_module(
                    hidden_states, output_hidden_states=output_hidden_states, output_attentions=output_attentions
                )
...
class Sam2TwoWayTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return queries, keys, intermediate_hidden_states
...
class Sam2MaskDecoder(nn.Module):
    def __init__(self, config: Sam2MaskDecoderConfig):
        ...
        self.num_query_tokens = config.num_queries
        self.num_feature_levels = config.num_feature_levels

        self.iou_token = nn.Embedding(1, self.hidden_size)
        self.mask_tokens = nn.Embedding(self.num_mask_tokens, self.hidden_size)
        self.query_tokens = nn.Embedding(config.num_queries, self.hidden_size)
        ...
        for i in range(self.num_feature_levels):
            mlps_list += [Sam2FeedForward(self.hidden_size, self.hidden_size, self.hidden_size // divde_factors[i], 3)]
        self.query_prediction_pre_mlps = nn.ModuleList(mlps_list)

        mlps_list = []
        for i in range(self.num_feature_levels):
            mlps_list += [Sam2FeedForward(self.hidden_size // divde_factors[i], self.hidden_size, self.hidden_size, 3)]
        self.query_prediction_post_mlps = nn.ModuleList(mlps_list)
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        Args:
            ...
            return_dict (`bool`, *optional*):
                Whether to return a dictionary of outputs instead of a tuple.
            **kwargs: Unpack[TransformersKwargs]:
        ...
        if cond_lens is not None:
            ...
            high_resolution_features = (
                torch.cat(
                    [
                        image_embedding.unsqueeze(0).repeat(cond_len, 1, 1, 1)
                        for image_embedding, cond_len in zip(high_resolution_features[0], cond_lens)
                    ]
                ),
                torch.cat(
                    [
                        image_embedding.unsqueeze(0).repeat(cond_len, 1, 1, 1)
                        for image_embedding, cond_len in zip(high_resolution_features[1], cond_lens)
                    ]
                ),
            )
        ...
        image_embeds = image_embeds.repeat_interleave(point_batch_size, dim=0)
        ...
        point_embeddings, image_embeds, transformer_intermediate_hidden_states = transformer_outputs
        iou_token_out = point_embeddings[:, :, 1, :]
        mask_tokens_out = point_embeddings[:, :, 2 : (2 + self.num_mask_tokens), :]
        query_tokens_out = point_embeddings[
            :, :, (2 + self.num_mask_tokens) : (2 + self.num_mask_tokens + self.num_query_tokens), :
        ]
        transformer_intermediate_hidden_states = [
            x[
                :,
                :,
                (2 + self.num_mask_tokens) : (2 + self.num_mask_tokens + self.num_query_tokens),
            ...
            .reshape(batch_size * point_batch_size, self.num_query_tokens, self.hidden_size)
            .transpose(0, 1)
            for x in transformer_intermediate_hidden_states
        ]
        transformer_intermediate_hidden_states.append(
            query_tokens_out.reshape(batch_size * point_batch_size, self.num_query_tokens, self.hidden_size).transpose(
                0, 1
            )
        )
        ...
        image_embeds = image_embeds.transpose(2, 3).view(batch_size * point_batch_size, num_channels, height, width)
        ...
        upscaled_embedding = self.upscale_conv1(image_embeds) + feat_s1
        ...
        multi_masks = (hyper_in @ upscaled_embedding).view(batch_size, point_batch_size, -1, height, width)

        # multi-scale predictions
        for i in range(self.num_feature_levels):
            upscaled_embedding = upscaled_embeddings[i]
            for cur_hidden_states in transformer_intermediate_hidden_states:
                cur_pre_mlp = self.query_prediction_pre_mlps[i]
                cur_post_mlp = self.query_prediction_post_mlps[i]
                cur_hidden_states = cur_pre_mlp(cur_hidden_states)
                mask_predictions = torch.einsum("qbd,bdhw->bqhw", cur_hidden_states, upscaled_embedding)
                cur_hidden_states = cur_post_mlp(cur_hidden_states)
                intermediate_hidden_states += (cur_hidden_states,)
                intermediate_mask_predictions += (mask_predictions,)
        ...
        if multimask_output:
            ...
            multi_masks = multi_masks[:, :, mask_slice, :, :]
            iou_pred = iou_pred[:, :, mask_slice]
        elif self.dynamic_multimask_via_stability and not self.training:
            mask_slice = slice(0, 1)
            multi_masks, iou_pred = self._dynamic_multimask_via_stability(multi_masks, iou_pred)
        else:
            mask_slice = slice(0, 1)
            multi_masks = multi_masks[:, :, mask_slice, :, :]
            iou_pred = iou_pred[:, :, mask_slice]
        ...
        if return_dict:
            return Sam2MaskDecoderOutput(
                intermediate_hidden_states=intermediate_hidden_states,
                masks_queries_logits=intermediate_mask_predictions,
                last_hidden_state=query_tokens_out,
                hidden_states=transformer_intermediate_hidden_states,
                attentions=None,
...
class Sam2VisionRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: Sam2Config, rope_feat_sizes: List[int]):
        super().__init__()
...
def apply_rotary_pos_emb_2d(
    ...
    key_cos: Optional[torch.Tensor] = None,
    key_sin: Optional[torch.Tensor] = None,
    repeat_freqs_q: bool = False,
    ...
    if repeat_freqs_q and query_cos.shape[-2] != q.shape[-2]:
        assert query_cos.shape[-2] <= q.shape[-2], "Query cosine shape must be smaller than query shape"
        query_cos = query_cos.repeat(1, 1, q.shape[-2] // query_cos.shape[-2], 1)
        query_sin = query_sin.repeat(1, 1, q.shape[-2] // query_sin.shape[-2], 1)

    q_embed = q.float()  # force upscale to float32 as in the original implementation
    q_embed = (q_embed * query_cos) + (rotate_pairwise(q_embed) * query_sin)

    if k_rot.shape[-2] == 0 or (key_cos is None and key_sin is None):
        # Handle case where keys might be empty due to dropout
        return q_embed.type_as(q), torch.cat([k_rot, k_pass], dim=-2)

    if repeat_freqs_k and key_cos.shape[-2] != k_rot.shape[-2]:
        assert key_cos.shape[-2] <= k_rot.shape[-2], "Key cosine shape must be smaller than key shape"
        key_cos = key_cos.repeat(1, 1, k_rot.shape[-2] // key_cos.shape[-2], 1)
        key_sin = key_sin.repeat(1, 1, k_rot.shape[-2] // key_sin.shape[-2], 1)

    # Apply rotary embedding to keys
    k_embed = k_rot.float()  # force upscale to float32 as in the original implementation
    k_embed = (k_embed * key_cos) + (rotate_pairwise(k_embed) * key_sin)
    # Concatenate back to full shape
    k_embed = torch.cat([k_embed.type_as(k), k_pass], dim=-2)
    return q_embed.type_as(q), k_embed


class Sam2RoPEAttention(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.config = config
        self.hidden_size = hidden_size if hidden_size is not None else config.memory_attention_hidden_size
        ...
        self.rope_q_repeat = rope_q_repeat
    ...
    def forward(
        ...
        key: torch.Tensor,
        value: torch.Tensor,
        query_rope_position_embeddings: tuple[torch.Tensor, torch.Tensor],
        key_rope_position_embeddings: Optional[tuple[torch.Tensor, torch.Tensor]] = None,
        num_k_exclude_rope: int = 0,
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        query_cos, query_sin = query_rope_position_embeddings
        key_cos, key_sin = key_rope_position_embeddings
        ...
        query, key = apply_rotary_pos_emb_2d(
            ...
            key_cos,
            key_sin,
            repeat_freqs_q=self.rope_q_repeat,
...
class Sam2MemoryAttentionLayer(nn.Module):
    def __init__(self, config: Sam2Config):
        ...
        self.self_attn = Sam2RoPEAttention(config)
        self.cross_attn = Sam2RoPEAttention(
            config, kv_in_dim=config.memory_encoder_output_channels, rope_q_repeat=True, rope_k_repeat=True
        )
    ...
    def forward(
        ...
        keys: Tensor,
        key_point_embedding: Tensor,
        query_rope_position_embeddings: tuple[Tensor, Tensor],
        num_k_exclude_rope: int = 0,
    ) -> torch.Tensor:
        # self-attention
        residual = queries
        ...
        queries, _ = self.self_attn(
            ...
            key=queries,
            value=queries,
            query_rope_position_embeddings=query_rope_position_embeddings,
            key_rope_position_embeddings=query_rope_position_embeddings,
        )
        queries = residual + self.dropout1(queries)
        ...
        queries, _ = self.cross_attn(
            ...
            value=keys,
            query_rope_position_embeddings=query_rope_position_embeddings,
            key_rope_position_embeddings=(None, None),  # key does not have rope position embeddings
            num_k_exclude_rope=num_k_exclude_rope,
        )
        queries = residual + self.dropout2(queries)

        # MLP
        residual = queries
        queries = self.layer_norm3(queries)
        queries = self.linear2(self.dropout(self.activation(self.linear1(queries))))
        queries = residual + self.dropout3(queries)
        return queries
...
class Sam2MemoryAttention(nn.Module):
    def __init__(self, config: Sam2Config):
        ...
        self.query_rotary_emb = (
            nn.ModuleList(
                [
                    Sam2VisionRotaryEmbedding(config=config, rope_feat_sizes=rope_feat_sizes)
                    for rope_feat_sizes in config.memory_attention_query_rope_feat_sizes
                ]
            )
            if isinstance(config.memory_attention_query_rope_feat_sizes[0], list)
            else Sam2VisionRotaryEmbedding(
                config=config, rope_feat_sizes=config.memory_attention_query_rope_feat_sizes
            )
        )
    ...
    def forward(
        ...
        query_rope_position_embeddings = self.query_rotary_emb[scale_idx]()
        for layer in self.layers:
            output = layer(
...
class Sam2MemoryFuserCXBlock(GradientCheckpointingLayer):
    def __init__(self, config: Sam2Config):
        ...
        self.layer_norm = Sam2LayerNorm(config.memory_fuser_embed_dim, eps=1e-6, data_format="channels_first")
...
class Sam2MaskDownSamplerLayer(nn.Module):
    def __init__(self, config: Sam2Config, in_channels: int, out_channels: int):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channels,
            out_channels,
        ...
        self.layer_norm = Sam2LayerNorm(out_channels, eps=1e-6, data_format="channels_first")
...
class Sam2FeatureDownSamplerLayer(nn.Module):
    def __init__(self, config: Sam2Config, in_channels: int, out_channels: int):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channels,
            out_channels,
            kernel_size=config.feature_downsampler_kernel_size,
            stride=config.feature_downsampler_stride,
            padding=config.feature_downsampler_padding,
        )
        self.layer_norm = Sam2LayerNorm(out_channels, eps=1e-6, data_format="channels_first")
        self.activation = ACT2FN[config.feature_downsampler_hidden_act]
...
class Sam2MaskDownSampler(nn.Module):
    ...
    def __init__(self, config: Sam2Config, total_stride: int = 16):
        super().__init__()

        num_layers = int(math.log2(total_stride) // math.log2(config.mask_downsampler_stride))

        self.layers = nn.ModuleList()
        self.activation = ACT2FN[config.mask_downsampler_hidden_act]
        mask_in_chans = config.mask_downsampler_input_channels
        mask_out_chans = config.mask_downsampler_output_channels
        for _ in range(num_layers):
            mask_out_chans = mask_in_chans * (config.mask_downsampler_stride**2)
            self.layers.append(Sam2MaskDownSamplerLayer(config, mask_in_chans, mask_out_chans))
...
class Sam2FeatureDownSampler(nn.Module):
    """
    Progressively downsample a feature map by total_stride, each time by stride.
    Note that LayerNorm is applied per *token*, like in ViT.
    ...
    def __init__(self, config: Sam2Config, total_stride: int = 16):
        ...
        num_layers = int(math.log2(total_stride) // math.log2(config.feature_downsampler_stride))

        self.layers = nn.ModuleList()
        self.activation = ACT2FN[config.feature_downsampler_hidden_act]
        feature_in_chans = config.memory_encoder_hidden_size
        feature_out_chans = config.memory_encoder_hidden_size
        for _ in range(num_layers):
            feature_out_chans = feature_in_chans * (config.feature_downsampler_stride**2)
            self.layers.append(Sam2FeatureDownSamplerLayer(config, feature_in_chans, feature_out_chans))
            feature_in_chans = feature_out_chans

        self.final_conv = nn.Conv2d(feature_out_chans, config.memory_encoder_output_channels, kernel_size=1)
...
class Sam2MemoryEncoder(nn.Module):
    def __init__(self, config: Sam2Config):
        ...
        self.use_mask_guidance = config.use_mask_guidance
        self.use_class_guidance = config.use_class_guidance
        ...
        if self.use_mask_guidance:
            self.mask_downsampler = Sam2MaskDownSampler(
                config,
                (
                    config.mask_downsampler_scale_strides[0]
                    if config.mask_downsampler_scale_strides is not None
                    else config.mask_downsampler_total_stride
                ),
            )
            self.memory_fuser = Sam2MemoryFuser(config)
        self.vision_downsampler = (
            nn.ModuleList(
                [
                    Sam2FeatureDownSampler(config, scale_stride)
                    for scale_stride in config.feature_downsampler_scale_strides
                ]
            )
            if config.feature_downsampler_scale_strides is not None
            else nn.ModuleList([Sam2FeatureDownSampler(config, config.feature_downsampler_total_stride)])
        )
        self.projection = nn.Conv2d(hidden_size, hidden_size, kernel_size=1)

        self.position_encoding = Sam2SinePositionEmbedding(num_pos_feats=output_channels // 2, normalize=True)
    ...
    def forward(
        ...
        vision_features = self.vision_downsampler[scale_idx](vision_features)
        if self.use_mask_guidance:
            if self.use_class_guidance:
                scores = class_logits.softmax(dim=-1)[..., :-1].max(dim=-1).values  # [B, Q]
                score_bias = torch.log(scores.clamp_min(1e-6))[..., None, None]  # [B, Q, 1, 1]
                mask_weights = (mask_logits + score_bias).softmax(dim=1)  # [B, Q, H, W]
            else:
                mask_weights = mask_logits.softmax(dim=1)

            mask_logits = (mask_weights * mask_logits.sigmoid()).sum(dim=1, keepdim=True)  # [B, 1, H, W]
            mask_logits = mask_logits * self.sigmoid_scale_for_mem_enc + self.sigmoid_bias_for_mem_enc
            mask_logits = self.mask_downsampler(mask_logits)
            vision_features = vision_features + mask_logits
...
class Sam2Model(Sam2PreTrainedModel):
    ...
    def forward(
        ...
        video_inference_session: Sam2VideoInferenceSession,
        frame_idx: Optional[int] = None,
        frame: Optional[torch.Tensor] = None,
        reverse: bool = False,
        **kwargs,
        ...
        #     video_inference_session = Sam2VideoInferenceSession(
        #         videos=videos,  # [b * t, h, w, c]
        #         video_height=videos.shape[1],
        #         video_width=videos.shape[2],
        #         inference_device=self.device,
        #         inference_state_device=self.device,
        #         video_storage_device=self.device,
        #         dtype=self.dtype,
        #     )
        #     video_inference_session.obj_id_to_idx(1)

        if frame is not None:
            frame_idx = video_inference_session.add_new_frame(frame, frame_idx)

        if frame is not None and video_inference_session.get_obj_num() == 0:
            raise ValueError("No objects are provided for tracking; please add inputs first.")

        num_objects = video_inference_session.get_obj_num()
        ...
        for obj_idx in range(num_objects):
            obj_id = video_inference_session.obj_idx_to_id(obj_idx)
            has_new_inputs = obj_id in video_inference_session.obj_with_new_inputs
            has_cond_output = frame_idx in video_inference_session.output_dict_per_obj[obj_idx]["cond_frame_outputs"]
            ...
            if (not has_new_inputs) and has_cond_output:
                pred_masks = video_inference_session.get_output(
                ...
                if has_new_inputs:
                    is_init_cond_frame = frame_idx not in video_inference_session.frames_tracked_per_obj[obj_idx]
                    if is_init_cond_frame:
                        reverse = False
                    point_inputs = video_inference_session.point_inputs_per_obj[obj_idx].get(frame_idx, None)
                    mask_inputs = video_inference_session.mask_inputs_per_obj[obj_idx].get(frame_idx, None)
                    if point_inputs is not None or mask_inputs is not None:
                        video_inference_session.obj_with_new_inputs.remove(obj_id)

                current_out = self._run_single_frame_inference(
                    video_inference_session=video_inference_session,
                    obj_idx=obj_idx,
                    frame_idx=frame_idx,
                ...
                video_inference_session.store_output(
            ...
            if not is_init_cond_frame:
                ...
                video_inference_session.frames_tracked_per_obj[obj_idx][frame_idx] = {"reverse": reverse}
    ...
    def _prepare_vision_features(
        ...
        if cached_features := video_inference_session.cache.get_vision_features(frame_idx):
            ...
            image_batch = video_inference_session.get_frame(frame_idx).unsqueeze(0)  # Add batch dimension
            vision_feats, vision_pos_embeds, _, _ = self.get_image_features(image_batch)
            # Cache features
            video_inference_session.cache.cache_vision_features(
    ...
    def _single_frame_forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        """
        if not ((pixel_values is None) ^ (image_embeds is None)):
            raise ValueError("Exactly one of pixel_values or image_embeds must be provided.")
        ...
        batch_size = pixel_values.shape[0] if pixel_values is not None else image_embeds[-1].shape[0]
        ...
        if input_points is None and input_boxes is None:
            ...
            input_points = torch.zeros(
                batch_size, 1, 1, 2, dtype=image_embeds[-1].dtype, device=image_embeds[-1].device
            )
            input_labels = -torch.ones(batch_size, 1, 1, dtype=torch.int32, device=image_embeds[-1].device)
        ...
        _, _, _, _, _, low_res_multimasks, iou_scores, sam_output_tokens, object_score_logits = self.mask_decoder(
            image_embeds=image_embeds[-1],
            image_positional_embeddings=image_positional_embeddings,
            sparse_prompt_embeddings=sparse_embeddings,
            dense_prompt_embeddings=dense_embeddings,
            multimask_output=multimask_output,
            high_resolution_features=image_embeds[:-1],
            attention_similarity=attention_similarity,
            target_embedding=target_embedding,
            return_dict=False,
            **kwargs,
    ...
    def _use_mask_as_output(
        ...
        object_pointer = self._single_frame_forward(
            ...
            image_embeds=high_res_features + [backbone_features],
        ).object_pointer
    ...
    def _gather_memory_frame_outputs(
        ...
        conditioning_outputs = video_inference_session.output_dict_per_obj[obj_idx]["cond_frame_outputs"]
        ...
        for relative_temporal_offset in range(self.num_maskmem - 1, 0, -1):
            ...
            output_data = video_inference_session.output_dict_per_obj[obj_idx]["non_cond_frame_outputs"].get(
    ...
    def _get_object_pointers(
        ...
        for t_diff_offset in range(1, max_object_pointers_to_use):
            ...
            # check if the output is already stored without using get_output to avoid unnecessary memory transfers between CPU and GPU
            out_data = video_inference_session.output_dict_per_obj[obj_idx]["non_cond_frame_outputs"].get(
                ref_frame_idx, None
            )
    ...
    def _prepare_memory_conditioned_features(
        ...
        temporal_positions_and_previous_outputs = self._gather_memory_frame_outputs(
            video_inference_session, obj_idx, frame_idx, track_in_reverse_time
        )
        ...
        temporal_offsets, pointer_tokens, max_object_pointers_to_use = self._get_object_pointers(
            video_inference_session, obj_idx, frame_idx, num_total_frames, device, track_in_reverse_time, streaming
        )
    ...
    def _run_single_frame_inference(
        ...
        current_vision_feats, current_vision_pos_embeds = self._prepare_vision_features(
            video_inference_session, frame_idx, batch_size
        )
        ...
        if mask_inputs is not None:
            ...
            pix_feat = self._prepare_memory_conditioned_features(
                ...
                num_total_frames=video_inference_session.num_frames,
            ...
            sam_outputs = self._single_frame_forward(
                ...
                image_embeds=high_res_features + [pix_feat],
                multimask_output=multimask_output,
            )
    ...
    def _encode_new_memory(
        ...
        maskmem_features, maskmem_pos_enc, _ = self.memory_encoder(
            pix_feat,
            mask_for_mem,
        )
    ...
    @auto_docstring(
        ...
        Yields Sam2ImageSegmentationOutput for each frame.
        """
    )
    def propagate_in_video_iterator(
        ...
        max_num_frame_to_track: Optional[int] = None,
        reverse: bool = False,
    ) -> Iterator[Sam2ImageSegmentationOutput]:
        r"""
        video_inference_session (`Sam2VideoInferenceSession`):
            The video inference session object.
        ...
        max_num_frame_to_track (`int`, *optional*):
            The maximum number of frames to track.
        reverse (`bool`, *optional*, defaults to `False`):
            Whether to propagate in reverse.
        """
        num_frames = video_inference_session.num_frames
        ...
        if start_frame_idx is None:
            ...
            frames_with_inputs = [
                frame_idx
                for obj_output_dict in video_inference_session.output_dict_per_obj.values()
        ...
        if max_num_frame_to_track is None:
            # default: track all the frames in the video
            max_num_frame_to_track = num_frames
        if reverse:
            end_frame_idx = max(start_frame_idx - max_num_frame_to_track, 0)
            if start_frame_idx > 0:
                processing_order = range(start_frame_idx, end_frame_idx - 1, -1)
            else:
                processing_order = []  # skip reverse tracking if starting from frame 0
        else:
            end_frame_idx = min(start_frame_idx + max_num_frame_to_track, num_frames - 1)
            processing_order = range(start_frame_idx, end_frame_idx + 1)

        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            sam2_video_output = self(video_inference_session, frame_idx=frame_idx, reverse=reverse)
